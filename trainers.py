...
def preference_loss(policy_chosen_logps: torch.FloatTensor,
    ...
    if ipo:
        ...
        #print('logits',logits)
        #print('logsigmoid',F.logsigmoid(beta * logits))
    #print(logits,losses)
    #chosen_rewards = beta * (policy_chosen_logps - reference_chosen_logps).detach()
    chosen_rewards = chosen_rewards.detach()
    #print('reward',chosen_rewards)
...
class BasicTrainer(object):
    def __init__(self, policy: nn.Module, config: DictConfig, seed: int, run_dir: str, reference_model: Optional[nn.Module] = None, rank: int = 0, world_size: int = 1):
        ...
        self.tokenizer = transformers.AutoTokenizer.from_pretrained(tokenizer_name_or_path, token='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',cache_dir=get_local_dir(config.local_dirs))
    ...
    def concatenated_forward(self, model: nn.Module, batch: Dict[str, Union[List, torch.LongTensor]]) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        ...
        #print('input---',concatenated_batch['concatenated_input_ids'])
        
        if torch.isnan(all_logits).any():
            print('all_logits---',all_logits[-2:])
            print('all_label---',concatenated_batch['concatenated_input_ids'][-2:])
    ...
    def get_batch_metrics(self, batch: Dict[str, Union[List, torch.LongTensor]], loss_config: DictConfig, train=True):
        ...
        if loss_config.name in {'dpo', 'ipo'}:
            ...
            margins = (chosen_rewards - rejected_rewards).float()
            loss1 = -F.logsigmoid(chosen_rewards - rejected_rewards).float()

            chosen_rewards = all_gather_if_needed(chosen_rewards, self.rank, self.world_size)
            rejected_rewards = all_gather_if_needed(rejected_rewards, self.rank, self.world_size)
            reward_accuracies = all_gather_if_needed(reward_accuracies, self.rank, self.world_size)
            margins = all_gather_if_needed(margins, self.rank, self.world_size)
            loss1 = all_gather_if_needed(loss1, self.rank, self.world_size)

            metrics[f'rewards_{train_test}/chosen'] = chosen_rewards.cpu().numpy().tolist()
            metrics[f'rewards_{train_test}/rejected'] = rejected_rewards.cpu().numpy().tolist()
            metrics[f'rewards_{train_test}/accuracies'] = reward_accuracies.cpu().numpy().tolist()
            #metrics[f'rewards_{train_test}/margins'] = (chosen_rewards - rejected_rewards).cpu().numpy().tolist()
            metrics[f'rewards_{train_test}/margins'] = margins.cpu().numpy().tolist()
            
            #print('margins',(chosen_rewards - rejected_rewards).cpu().numpy().tolist())
            #print('loss',(-F.logsigmoid(chosen_rewards - rejected_rewards)).cpu().numpy().tolist())

            #all_devices_losses = all_gather_if_needed(losses.detach(), self.rank, self.world_size)
            #metrics[f'rewards_{train_test}/loss'] = all_devices_losses.cpu().numpy().tolist()
        ...
        #print('value input----------:', losses.detach())
        all_devices_losses = all_gather_if_needed(losses.detach(), self.rank, self.world_size)
        metrics[f'loss/{train_test}'] = all_devices_losses.cpu().numpy().tolist()
        #print('value output----------:', all_devices_losses)
        #print('value output to list----------:', all_devices_losses.cpu().numpy().tolist())
    ...
    def train(self):
        ...
        self.optimizer = getattr(torch.optim, self.config.optimizer)(self.policy.parameters(), lr=self.config.lr,weight_decay = 0.0001)
        ...
        for batch in self.train_iterator:
            ...
            if self.example_counter % self.config.eval_every == 0 and (self.example_counter > 0 or self.config.do_first_eval):
                ...
                for eval_batch in (tqdm.tqdm(self.eval_batches, desc='Computing eval metrics') if self.rank == 0 else self.eval_batches):
                    ...
                    #print('local_eval_batch',local_eval_batch['prompt'][:2])
                    with torch.no_grad():
                ...
                if self.example_counter >= 100000:
            ...
            for microbatch_idx in range(self.config.gradient_accumulation_steps):
                #print('microbatch_idx',self.config.gradient_accumulation_steps)
                #print('microbatch_idx',microbatch_idx)
            ...
            learn_rate = self.scheduler.get_lr()[0]

            step_time = time.time() - start_time
            examples_per_second = self.config.batch_size / step_time
            batch_metrics['examples_per_second'].append(examples_per_second)
            batch_metrics['grad_norm'].append(grad_norm)
            batch_metrics['lr'].append(learn_rate)
...
class FSDPTrainer(BasicTrainer):
    ...
    def save(self, output_dir=None, metrics=None):
        ...
        try:
            print('start policy', self.rank)
            save_policy = FullStateDictConfig(offload_to_cpu=True, rank0_only=True)
            
            with FSDP.state_dict_type(self.policy, StateDictType.FULL_STATE_DICT, state_dict_config=save_policy):
                policy_state_dict = self.policy.state_dict()

            if self.rank == 0:# and metrics['rewards_eval/accuracies']>self.best_acc:
                #self.best_acc = metrics['rewards_eval/accuracies']
