...
def get_hh(split: str, silent: bool = False, cache_dir: str = None) -> Dict[str, Dict[str, Union[List[Tuple[int, int]], List[str], str]]]:
    ...
    if split == 'train':
        #split = 'train[:1000]'
        print('---split',split)
        dataset = datasets.load_dataset('honggen/phi-harm', split='train',data_dir = 'hard',cache_dir=cache_dir)
        #dataset = datasets.load_dataset('Anthropic/hh-rlhf', split=split,data_dir = 'harmless-base',cache_dir=cache_dir)
        #dataset = datasets.load_dataset('honggen/shp2-phi15-centroid', split='train',data_dir = None, cache_dir=cache_dir)
    else:
        print('---split',split)
        #dataset = datasets.load_dataset('honggen/SHP-2-all', split=split,data_dir =None, cache_dir=cache_dir)
        dataset = datasets.load_dataset('Anthropic/hh-rlhf', split=split,data_dir = 'helpful-base', cache_dir=cache_dir)
    print('done')
        ...
        for prompt, responses, pairs, sft_target, truncation_mode in flat_data:
            ...
            if sft_mode:
                ...
                for p in pairs:
                    ...
                    batch.append(batch_element)
                    example_idx += 1
                    #print(' number example_idx',example_idx)
                    if len(batch) == batch_size:
                        #print('++++++++++++n',example_idx)
                        yield collate_fn(batch)
                        if n_examples is not None and example_idx >= n_examples:
                            print('error-----------------------------',n_examples)
                            print(' number example_idx',example_idx)
                            if not silent:
